...
export interface RecallResult {
   ...
   * Contains persona + scene navigation (static content identical across
   * turns, forming a stable cacheable prefix).
   *
   * Real size: a persona generated by the built-in template is <2000 chars;
   * end-to-end measurement (persona + scene nav) sits at ~1.5–2k chars and is
   * byte-identical across turns — see prependSystemContext-realistic.test.ts.
   * (The 477-char figure elsewhere is a minimal fixture for boundary-position
   * tests, not the production scale.)
   *
   * Fixes #120 (secondary): previously these were in appendSystemContext
   * (after CACHE_BOUNDARY), so the provider re-billed this static content
   * as new tokens every turn instead of serving it from the prefix cache.
   */
